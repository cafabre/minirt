...
/*   Updated: 2026/01/12 15:58:58 by cafabre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	dispatch_sp(char **tab, t_scene *s, t_coords *c_list, t_data *data)
{
	...
	if (!c_list->coords || !ft_isnumber(tab[2])
		|| ft_atof(tab[2]) < 0 || !c_list->coords_r)
...
static bool	dispatch_pl(char **tab, t_scene *s, t_coords *c_list, t_data *data)
{
	...
	new_pl = create_pl(parse_vector(c_list->coords, 1, data),
			parse_vector(c_list->coords_r, 2, data),
			parse_vector(c_list->coords_r2, 3, data));
	if (!new_pl)
...
static bool	dispatch_cy(char **tab, t_scene *s, t_coords *c_list, t_data *data)
{
	...
	if (!c_list->coords || !c_list->coords_r || !ft_isnumber(tab[3])
		|| ft_atof(tab[3]) < 0 || !ft_isnumber(tab[4])
		|| ft_atof(tab[4]) < 0 || !c_list->coords_r2)
		return (ret_error(data, ERR_INVALID_CYLINDER_DATA));
	new_cy = create_cy(parse_vector(c_list->coords, 1, data),
			parse_vector(c_list->coords_r, 2, data), ft_atof(tab[3]),
			ft_atof(tab[4]), parse_vector(c_list->coords_r2, 3, data));
	if (!new_cy)
